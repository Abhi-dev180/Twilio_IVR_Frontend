...
export default function Dashboard({ token, setToken }) {
  ...
  if (activeTotalAttempts > 0) {
    if (
      activeTotalAttempts === 1 &&
      activeAttempts[0].test_value &&
      activeAttempts[0].test_value.includes(':')
    ) {
      const currentCode = parseInt(activeAttempts[0].test_value.split(':')[1], 10) || 0;
      progressPercent = Math.min(100, Math.max(0, Math.round((currentCode / 999) * 100)));
      if (['completed', 'failed'].includes(activeAttempts[0].status)) {
  ...
  const showProgress =
    campaignRunning ||
    (activeTotalAttempts > 0 && progressPercent < 100) ||
    (activeTotalAttempts === 1 && activeAttempts[0].status === 'active');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 text-slate-800 font-sans relative overflow-x-hidden flex flex-col justify-between selection:bg-blue-500 selection:text-white">
      {/* Background Subtle Gradient Blobs */}
      <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-blue-300/25 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute top-1/3 right-10 w-[400px] h-[400px] bg-indigo-300/20 rounded-full blur-[100px] pointer-events-none" />
      ...
      {/* Main Wrapper */}
      <div className="max-w-[1600px] w-full mx-auto px-4 sm:px-6 lg:px-10 py-6 sm:py-8 relative">
        {/* Header Section */}
        <motion.header
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
          className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/80 border border-slate-200 backdrop-blur-xl p-5 sm:p-6 rounded-2xl shadow-lg"
        >
          <div className="flex items-center gap-4 min-w-0 w-full md:w-auto">
            <motion.div
              whileHover={{ rotate: 6, scale: 1.06 }}
              className="p-3 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl text-white shadow-lg shadow-blue-500/25 ring-1 ring-white/40 shrink-0"
            >
              <Activity className="w-6 h-6 sm:w-7 sm:h-7" />
            </motion.div>

            <div className="min-w-0 flex-1">
              <h1 className="bg-gradient-to-r from-slate-900 via-slate-700 to-blue-600 bg-clip-text text-xl sm:text-2xl lg:text-3xl font-extrabold tracking-tight text-transparent truncate">
                Twilio IVR QA Platform
              </h1>

              <div className="mt-1 flex items-center gap-2">
                <span className="h-1.5 w-1.5 shrink-0 animate-pulse rounded-full bg-emerald-500" />
                <p className="text-xs font-medium tracking-wide text-slate-500 truncate">
                  Automated voice testing platform
                </p>
              </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end flex-wrap">
            <div className="flex items-center gap-2 px-3.5 py-2 bg-emerald-50 border border-emerald-200 rounded-xl shadow-sm">
              <span className="relative flex h-2.5 w-2.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
                <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500" />
              </span>
              <span className="text-xs font-semibold text-emerald-700 tracking-wide uppercase whitespace-nowrap">
                System Active
              </span>
            </div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={handleLogout}
              className="flex items-center gap-2 px-4 py-2 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 rounded-xl text-xs font-semibold transition-all shadow-sm"
              title="Sign Out"
            >
              <LogOut className="w-4 h-4" /> Logout
            </motion.button>
      ...
      {/* Footer */}
      <footer className="mt-12 py-6 border-t border-slate-200 bg-white/60 backdrop-blur-md">
        <div className="max-w-[1600px] mx-auto px-4 text-center text-xs text-slate-500 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="flex items-center gap-1.5 font-medium">
            <ShieldCheck className="w-4 h-4 text-blue-600" /> Twilio Voice Automated QA Platform
          </p>
          <p className="text-slate-400">Built for Matthew &amp; Partner</p>
        </div>
      </footer>
    </div>
  );
